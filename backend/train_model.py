"""
Train the credit-default model once and save it to disk (model.joblib).
Run this ONCE locally:  python train_model.py
The API loads the saved model at startup (fast, no retraining per request).
"""
...
def engineer(X):
    ...
    dcols= ["PAY_0","PAY_2","PAY_3","PAY_4","PAY_5","PAY_6"]
    Xf["utilization"]   = (X["BILL_AMT1"]/X["LIMIT_BAL"]).clip(0,5)
    Xf["pay_to_bill"]   = (X[pay].sum(axis=1)/(X[bill].sum(axis=1)+1))
    Xf["max_delay"]     = X[dcols].max(axis=1)
    Xf["n_months_late"] = (X[dcols]>0).sum(axis=1)
    return Xf
...
if __name__ == "__main__":
    ...
    joblib.dump({"model": model, "features": list(Xf.columns)}, "model.joblib")
